...
module.exports = class CSSOutlet {
    ...
    _filterVariations(entries, variations) {
        const normalizedEntries = Array.from(entries.values()).reduce(
            (normalized, entry) => {
        ...
        const variationalEntries = Object.keys(normalizedEntries).reduce(
            (variational, key) => {
...
function combineCss(cssEntries, outputFileName = '') {
    ...
    map.sourcesContent = map.sources.map((id, index) => {
